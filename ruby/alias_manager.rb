...
# Driver code / UI. Encapsulating a code to prompt user for name.
...
name_storage = {}
	...
	name_storage[:real_name] = name
		...
		name_storage[:fake_id] = encrypt_name(name)

	end	

	puts "jk! #{name_storage[:fake_id]} is really #{name_storage[:real_name]}!"
end
runner
